...
class prm_generator_laststage(nn.Module):
    def __init__(self, in_channel=64, norm='in', num_cls=4):
        ...
        self.embedding_layer = nn.Sequential(
                            general_conv3d(in_channel*Modal_count, int(in_channel//4), k_size=1, padding=0, stride=1),
...
class modal_fusion(nn.Module):
    def __init__(self, in_channel=64):
        ...
        self.weight_layer = nn.Sequential(
                            nn.Conv3d(Modal_count*in_channel+1, 128, 1, padding=0, bias=True),
                            nn.LeakyReLU(negative_slope=0.2, inplace=True),
                            nn.Conv3d(128, Modal_count, 1, padding=0, bias=True))
...
class region_aware_modal_fusion(nn.Module):
    def __init__(self, in_channel=64, norm='in', num_cls=4):
        ...
        self.short_cut = nn.Sequential(
                        general_conv3d(in_channel*Modal_count, in_channel, k_size=1, padding=0, stride=1),
                        general_conv3d(in_channel, in_channel, k_size=3, padding=1, stride=1),
                        general_conv3d(in_channel, in_channel//2, k_size=1, padding=0, stride=1))

        self.clsname_list = ['BG'] * num_cls ##BRATS2020 and BRATS2018
        self.clsname_list = ['BG'] * num_cls ##BRATS2015
    ...
    def forward(self, x, prm, mask):
        ...
        feats = []
        for l in range(Modal_count):
            feats.append(y[:, l:l+1, ...] * prm)

        modal_feat = torch.stack(feats, dim=1)
